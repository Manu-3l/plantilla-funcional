...
object Benchmarks {
  ...
  def benchmarkingVectores():Unit = {
    ...
    println(s"Benchmarking para vectores de tamaño 2^i:")

    for (i <- 1 to 7) {
      // Generar vectores aleatorios
      val v1 = App.vectorAlAzar(math.pow(2, i).toInt, 100)
      val v2 = App.vectorAlAzar(math.pow(2, i).toInt, 100)
  ...
  //Benchmarking para la multiplicación de matrices con metodo estandar
  def benchmarkingMultMatriz():Unit = {
    ...
    for (i <- 1 to 7) {
      ...
      val seqTime = withWarmer(new Warmer.Default) measure {
        App.multMatriz(m1, m2)
      }

      val parTime = withWarmer(new Warmer.Default) measure {
        App.multMatrizPar(parM1, parM2)
      }
  ...
  //Benchmarking para la multiplicación de matrices metodo Recursivo
